...
function MasterCustomColor()
  ...
  local r, g, b
  if gfx.mouse_cap & 16 == 16 then  --Alt = reset
    r, g, b = 0, 0, 0
  else
...
local function drawMasterColorSwatch(self)
  local scale = drawScale or 1
  local radius = 6
  local cx = (self.drawx or self.x or 0) + (self.drawW or self.w or 0) / 2
  local cy = (self.drawy or self.y or 0) + (self.drawH or self.h or 0) / 2
  if scale > 1 then
    cx, cy, radius = cx * scale, cy * scale, radius * scale
  end

  local r, g, b = getMasterCol()
  if not (r and g and b) or (r == 0 and g == 0 and b == 0) then return end

  gfx.set(r / 255, g / 255, b / 255, 1)
  gfx.circle(cx, cy, radius, true)
  gfx.set(0, 0, 0, 0.3)
...
function applyCustCol(col)
  ...
  if mouseCap & 8 == 8 then  --Shift = update master color
    local r, g, b = table.unpack(col)
    if mouseCap & 16 == 16 then  --Shift+Alt = reset master color
      r, g, b = 0, 0, 0
    end
  ...
  elseif cursorContext2 ~= 0 and (mouseCap & 4 == 4) and (mouseCap & 16 == 16) then  --Ctrl+Alt = set take colors
    for selindex = 0, count-1 do
      ...
      if take then
        ...
        reaper.ColorToNative(table.unpack(col)) | 0x1000000)
      end
      reaper.UpdateItemInProject(sel_item)
    end

  elseif cursorContext2 ~= 0 and (mouseCap & 4 == 4) then  --Ctrl = set item colors
    for selindex = 0, count-1 do
      local sel_item = reaper.GetSelectedMediaItem(0, selindex)
      reaper.SetMediaItemInfo_Value(sel_item, 'I_CUSTOMCOLOR',
      reaper.ColorToNative(table.unpack(col)) | 0x1000000)
      reaper.UpdateItemInProject(sel_item)
    end
  end
...
--color gradient to children (HSL + complementary)
function applyChildren()
  ...
  local origSel = {}
  for i = 0, selCount-1 do origSel[#origSel+1] = r.GetSelectedTrack(0,i) end

  local CONFIG = {
    default   = { L = -0.35, S =  0.00 },  --no modifier (darken)
    ctrl      = { L = -0.25, S =  0.00 },  --Ctrl/Cmd = softer darken
    shift     = { L =  0.40, S =  0.00 },  --Shift = brighten
    ctrlshift = { L =  0.30, S =  0.00 },  --Ctrl+Shift = softer brighten
    alt       = { L =  0.00, S =  0.00 },  --Alt = same color for all
    comp      = { L =  0.00, S =  0.00 },  --Ctrl+Alt+Shift = complementary
  }
  local cap = (gfx and gfx.mouse_cap) or 0
  local modecfg = CONFIG.default
  if cap == 28 or cap == 20 then  --Ctrl+Alt+Shift
    modecfg = CONFIG.comp
  elseif cap == 12 then  --Ctrl+Shift
    modecfg = CONFIG.ctrlshift
  elseif (cap & 4) == 4 then
    modecfg = CONFIG.ctrl
  elseif (cap & 8) == 8 then
    modecfg = CONFIG.shift
  elseif (cap & 16) == 16 then
    modecfg = CONFIG.alt
  end

  --HSL helpers
  local function rgbToHsl(rr, gg, bb)
    local r_, g_, b_ = rr/255, gg/255, bb/255
    local maxc, minc = math.max(r_, g_, b_), math.min(r_, g_, b_)
    local h, s, l = 0, 0, (maxc + minc) / 2
    if maxc ~= minc then
      local d = maxc - minc
      s = (l>0.5) and (d/(2-maxc-minc)) or (d/(maxc+minc))
      if maxc==r_ then h=(g_-b_)/d + (g_<b_ and 6 or 0)
      elseif maxc==g_ then h=(b_-r_)/d +2
      else h=(r_-g_)/d +4 end
      h = h/6
    end
    return h,s,l
  end
  ...
  local function hslToRgb(h, s, l)
    ...
    return math.max(1,math.floor(r_*255+0.5)),
    math.max(1,math.floor(g_*255+0.5)),
    math.max(1,math.floor(b_*255+0.5))
  end

  local totalTracks = r.CountTracks(0)

  local function countChildren(parent)
    local parentDepth = r.GetTrackDepth(parent)
    local startIdx = r.CSurf_TrackToID(parent,false)
    local cnt = 0
    for i=startIdx,totalTracks-1 do
      local tr = r.GetTrack(0,i)
      if not tr then break end
      local d = r.GetTrackDepth(tr)
      if d <= parentDepth then break end
      if d == parentDepth +1 then cnt=cnt+1 end
    end
    return cnt
  end
  ...
  local function colorChildren(parent)
    local parentDepth = r.GetTrackDepth(parent)
    local startIdx = r.CSurf_TrackToID(parent,false)

    local baseColor = r.GetTrackColor(parent)
    if baseColor==0 then baseColor=0x909090 end
    local Cr,Cg,Cb = r.ColorFromNative(baseColor)
    if Cr==0 and Cg==0 and Cb==0 then Cr,Cg,Cb=1,1,1 end
    local h,s,l = rgbToHsl(Cr,Cg,Cb)

    local numChildren = countChildren(parent)
    local denom = (numChildren>0) and numChildren or 1

    local compH, compS, compL
    if modecfg == CONFIG.comp then
      compH = (h + 0.5) % 1
      compS = s
      compL = l
    end
    ...
    for i=startIdx,totalTracks-1 do
      ...
      if d == parentDepth +1 then
        stepN = stepN +1
        local newH, newS, newL
        if modecfg == CONFIG.comp then
          local t = stepN/denom
          newH = h*(1-t) + compH*t
          newS = s*(1-t) + compS*t
          ...
          newH = h
          newS = math.min(1, math.max(0, s + (modecfg.S or 0)/denom * stepN))
          newL = math.min(1, math.max(0, l + (modecfg.L or 0)/denom * stepN))
        end

        local rr,gg,bb = hslToRgb(newH,newS,newL)
        local childNative = r.GetTrackColor(tr)
        local isFolder = r.GetMediaTrackInfo_Value(tr,"I_FOLDERDEPTH")>0

        if not (isFolder and childNative~=0) then
          r.SetTrackColor(tr,r.ColorToNative(rr,gg,bb))
        end

        if isFolder then colorChildren(tr) end
      end
    end
  end

  for _,parent in ipairs(origSel) do colorChildren(parent) end

  r.PreventUIRefresh(-1)
  r.Undo_EndBlock("Gradient to children (HSL, complementary & modifiers)",0)

  r.Main_OnCommand(40297,0)
  for _,tr in ipairs(origSel) do r.SetTrackSelected(tr,true) end
end
...
local osOffs = (isWindows or isLinux) and -1 or 0

if isMac or isLinux then
  gfx.setfont(1, font, 11)
  gfx.setfont(2, font, 10)
  ...
  gfx.setfont(12, font, 20)
  gfx.setfont(13, font, 24)
  gfx.setfont(14, font, 28)  --used in : undocked palette title
  gfx.setfont(15, font, 25)
else  --Windows
  gfx.setfont(1, font, 13)
  gfx.setfont(2, font, 12)
  gfx.setfont(3, font, 14)
  gfx.setfont(4, font, 18)  --used in : undocked palette title
  gfx.setfont(5, font, 13)  --IMPORTANT: match TCP & EnvCP labels
  gfx.setfont(11, font, 26)
  gfx.setfont(12, font, 24)
  gfx.setfont(13, font, 30)
  gfx.setfont(14, font, 34)  --used in : undocked palette title
  ...
  for segment in str:gmatch("([^\n]*)\n?") do
    if segment ~= nil then
      if segment == "" then
        ...
        if wrap == true then
          local wrapped = textWrap(segment, drawScale * 105)
...
function ButtonLabel:new(parent, o)
  o = o or {}
  o.flow   = true
  o.text   = { str = o.text and o.text.str or "", col = {180,180,181}, align = 4, style = 1 }
  o.x      = 2
  o.h      = 30
  o.w      = o.w or 73
  o.border = o.border or ''
  ...
  if o.spinStyle == 'image' then
    ...
    if spinStyles[spinStyle].readout ~= false then
      local r = Readout:new(o,{x=self.buttonW,y=spinStyles[spinStyle].readout.y,w=self.w-(2*self.buttonW),h=spinStyles[spinStyle].readout.h,border='',
        text={str='  ---',align=5,val=o.value, col=spinStyles[spinStyle].readout.strCol, style=1},
...
function ParamRow:new(parent, valsTable, rowIdx)
  ...
  self.__index = self
  o.isParamTable = true
  local rowBgCol = (rowIdx % 2 == 0) and {46,50,47} or {0,0,0,35}
  local row = Element:new(parent,{x=0,y=rowIdx*24+7,w=453,h=24,color=rowBgCol})
  row.isParamTable = true
  local titleW = 114
  if valsTable.rows[rowIdx].img then
    local img = Element:new(row,{x=91,y=0,w=23,h=24,img=valsTable.rows[rowIdx].img})
    img.isParamTable = true
    titleW = 80
  end
  local title = Element:new(row,{x=0,y=0,w=titleW,h=24,text={str=valsTable.rows[rowIdx].text.str,style=1,align=6, col={180,180,181}}})
  title.isParamTable = true
  for i, v in ipairs(valsTable.columns) do
    local btn = Button:new(row,{x=44+i*82,y=0,w=81,h=24,img=valsTable.img,imgType=3,action=doFlagParam,param={valsTable.rows[rowIdx].param,v.visFlag}})
    btn.isParamTable = true
  end
  adoptChild(parent, o)
  setmetatable(o, self)
  return o
...
function ParamTable:new(parent, o)
  ...
  for i, v in ipairs(o.valsTable.columns) do
    local thisCol = {180,180,181}
    local colTitle = Element:new(o, {x=44+i*82,y=0,w=80,h=22,text={str=v.text.str,style=1,align=9,col=thisCol}})
    colTitle.isParamTable = true
  end
  for i=1, #o.valsTable.rows do ParamRow:new(o, o.valsTable, i) end
  for _, x in ipairs({124,206,288,370}) do
    local divider = Element:new(o, {x=x,y=0,w=1,h=o.h,color={254,254,254,30}})
    divider.isParamTable = true
  end
  adoptChild(parent, o)
  setmetatable(o, self)
  return o
...
function Swatch:new(parent,o)
  ...
  local label = Element:new(o, {x=100-(palStrW/2),y=20,w=palStrW+10,h=18,text={str=palStr,style=1,align=9},color={38,42,39}})
  adoptChild(parent,o)
  setmetatable(o, self)
  return o
end
...
function paramIdxGet(param)
  ...
  if param == 'tcp_indent' or param == 'ctrl_Param_301' or param == 'tcp_control_align'
  or param == 'tcp_customButton' or param == 'tcp_meterFlip' or param == 'ctrl_Param_101'
...
--read coloradjust from theme-config
function colorAdjustFromTheme()
...
--ACTIONS ------------------------------
function toggleHelp()
  doHelpVis(_helpR.visible == false)
end

function showInfoBox()
  if not (_systemInfo and _systemInfo.visible and _pageContainer and _helpR) then return end
  local marginX = 12
  local marginY = -33
  _systemInfo.x = _helpR.x + _helpR.w + (marginX - _systemInfo.w)
  _systemInfo.y = _pageContainer.y + marginY
end
...
function doHelpVis(visible)
  ...
  _systemInfo.visible = visible
  if visible then showInfoBox() end
  _gfxw,_gfxh = table.unpack(_desired_sizes[visible == true and 2 or 1])
  if _buttonHelp and _buttonHelp.text then
    if visible then
      _buttonHelp.text.col = {100, 180, 120}
    else
      _buttonHelp.text.col = {170,170,170}
    end
  end
...
function themeCheck()
  ...
  if theme ~= oldTheme or theme == nil then
    ...
    if theme ~= 'CSIX' or theme_version < 1 then
      ...
      _theme.text.style = 3
      _theme.y = -27
      _theme.text.col = {255,102,102}  --warning
    else
      _theme.text.col = {180,180,181}  --current
    end
...
function switchToAnyCSIX()
  ...
  if not theme_found then
    reaper.MB(translate('No fallback CSIX theme found.'), translate('Info:'), 0)
    Quit()
  else
    indexParams()
    redraw = 1
...
function get_csix_themes()  --themes for dropdown list
...
--CSIX settings management-1, end
--------------------------------------------------
...
function doActivePage()
  ...
  if _undockedRoot.visible ~= false then
    if isGenericTheme == false and ep == 6 then ep = 6 end
...
function doPageSpin(param)
  ...
  if isGenericTheme == true then
    ...
    limit = 6
    ep = editPage
    if _undockedRoot.visible == true then limit = 6 end
  end
...
function enforceTcpA(section, layout)  --basic narrow only
...
function updateTcpBtnOverlay()  --basic narrow only
  local theme = themeName()
  if theme and theme:find('CSIX%-BASIC%-Narrow') then
    local actualTcp = activeLayout.tcp
    local enforcedTcp = enforceTcpA('tcp', actualTcp)
...
function doActiveLayout(param)
  ...
  if param ~= nil then
    ...
    local layout = enforceTcpA(section, param[2])  --basic narrow only
  ...
  updateTcpPageOverlay()  --basic narrow only
  updateTcpBtnOlHelp()  --basic narrow only
  updateTcpBtnOverlay()  --basic narrow only
  paramGet = 1
  redraw = 1
end
...
function resetColorControls()
  ...
  paramGet = 1
  colorAdjustFromTheme()  --read coloradjust from csix theme-config
  redraw = 1
end
...
function showSettingPath()
  ...
  if isEmpty then
    reaper.MB(translate('No CSIX setting files found.\nUse the Save/Update buttons to export settings.\n'), translate('Info:'), 0)
  end
  local open_cmd = ({
  Win = 'start "" ',
  OSX = 'open ',
...
function showThemesPath()
  local folder = reaper.GetResourcePath() .. "/ColorThemes/"
  folder = folder:gsub("\\", "/")
  reaper.RecursiveCreateDirectory(folder, 0)
  local open_cmd = ({
  Win = 'start "" ',
...
function wipeThemeParams()
  ...
  if not f then
    ...
    translate('Could not open reaper-themeconfig.ini for reading.'),
    translate('Error'), 0)
  end

  local lines = {}
  local csix_names = {}
  for _, list in pairs(get_csix_themes()) do
    for _, t in ipairs(list) do
      csix_names[t.name:gsub("%.ReaperThemeZip$", ""):gsub("%.ReaperTheme$", "")] = true
    end
  end
  
  local keep_section, wipe_params = true, false
  for line in f:lines() do
    local section = line:match("^%[(.+)%]$")
    if section then
      if section:match("^CSIX") then
        keep_section = csix_names[section] or false
        wipe_params = csix_names[section] or false
      else
        keep_section, wipe_params = true, false
      end
      if keep_section then table.insert(lines, line) end
    elseif keep_section and (not wipe_params or not line:match("^param%d+=")) then
  ...
  if not f_out then
    return reaper.ShowMessageBox(
    translate('Could not open reaper-themeconfig.ini for writing.'),
    translate('Error'), 0)
  end
...
function wipeSaveAllSettings()
  ...
  --tryDelete('csixset.csixsetting') --keep for now
  tryDelete('csixset_all.csixsetting')  --reset default Merge
end
...
function totalReset()
  ...
  --reaper.ShowMessageBox(translate('Cleaned up parameter entries in root file:\r\nreaper-themeconfig.ini'), translate('Info:'), 0)
end
...
function csixList(mergeMode)  --switch/merge dropdown
  ...
  if selection > 0 then
    ...
    if selected_theme and selected_theme.type == 'theme' then
      if mergeMode and (mouse_cap & 4) == 4 then  --ctrl/cmd = prompt save all (merge mode)
        exportParamsAll()
      end
      ...
      if mergeMode then
        ...
        if reaper.file_exists(importFile) then
          ...
          reaper.MB(translate('No default preset named \"csixset_all\" was found.\nThe loaded theme will use its previous settings.\r\n\r\n'
          .. 'Tip:\nHold Ctrl (Cmd on Mac) to Save All Settings on\nthe fly when using Merge.\n'), translate('Theme not Merged'), 0)
        end
      end
    end
  end
...
--CSIX settings management-2, end
--------------------------------------------------
  ...
  if self.visible ~= false and crop ~= true then
    ...
    if self.img ~= nil or self.valsImage ~= nil then
      ...
      if self.imgType ~= nil and iDw ~= nil and self.imgType == 3 then
        if thisW ~= iDw/3 or thisH ~= iDh then  --width stretching needed
          local pad = drawScale * 10
          gfx.blit(i, 1, 0, (iDw/3)*(self.imgFrame or 0), 0, pad, iDh, thisX, thisY, pad, thisH)
          gfx.blit(i, 1, 0, ((iDw/3)*(self.imgFrame or 0)) + pad, 0, (iDw / 3) - (2 * pad), iDh, thisX + pad, thisY, thisW - (2 * pad), thisH)
          gfx.blit(i, 1, 0, (iDw/3)*(self.imgFrame or 0) + (iDw/3 - pad), 0, pad, iDh, thisX + thisW - pad, thisY, pad, thisH)
        else
    ...
    if self.text ~= nil then
      local fallback = translate('N/A')
      local txt = self.text
      --resolve function or val
      if type(txt) == "function" then
        local ok, res = pcall(txt)
        txt = ok and (type(res) == "table" and res or { str = tostring(res) }) or { str = fallback }
      end
      if type(txt) ~= "table" then txt = { str = tostring(txt or fallback) } end
      if type(txt.val) == "function" then
        local ok, v = pcall(txt.val)
        txt.str = ok and (v ~= nil and tostring(v) or fallback) or fallback
      end
      txt.str = txt.str or fallback
      local txtScaleOffs = drawScale == 2 and 1 or ''
      local tx, tw = thisX + (drawScale * textPadding), thisW - 2 * (drawScale * textPadding)
      --osOffs
      local drawY = thisY
      if type(txt) == "table" and txt.yOffs then
        drawY = drawY + txt.yOffs
      end
      text(txt.str, tx, drawY, tw, thisDrawH, txt.align, txt.col, txtScaleOffs .. (txt.style or 1), txt.lineSpacing, txt.vCenter, txt.wrap)
    end
...
--------------------------------------------------
--CSIX responsive settings (a,b,c)
...
function doTrackSound(self)
  ...
  local signal = getTrackSoundState(self.param[2], self.param[1], 1e-7)
...
local DOUBLE_CLICK_TIME = .4

function Element:mouseWheel(v)
  if self.isParamTable then return end
  if self.action and type(self.param) == 'table' then
    local paramName = self.param[1]
    local exclude = { ctrl_Param_101 = false, ctrl_Param_201 = true, glb_trackLabelCol = true, }  --prevent mousewheel
    if exclude[paramName] then return end
    self.action({paramName, v})
    root:doUpdateState()
    root:doParamGet()
    redraw = 1
  end
...
tcpTrackInfoVals        = {'OFF','Inline Swap','Inline Pin','Margin Swap','Margin Pin'}
...
transMetroBtnVals       = {'NONE','METRONOME','PREV / NEXT','ALL'}
...
helpR_help              = 'Customized Theme Adjuster for CSIX Basic Editions.\n\n'
                          ..'Parameters in blue indicate changes compared to the current theme\'s default settings.\n\n'
                          ..'Double-click any parameter to reset it, or use Reset This Theme to restore all built-in settings.\n\n'
                          ..'N/A usually means the feature isn\'t supported, or the parameter came from another theme that uses different settings. '
helpR_switchCSIX        = 'Switch to another CSIX Theme variant. '
helpR_mergeCSIX         = 'Select a CSIX Theme variant to load and MERGE with current parameters from the SAVE ALL Settings.\n\n'
                          ..'Tip:\nHolding Ctrl (Cmd on Mac) will prompt to save or update settings used for merging.\n\n'
...
helpR_layoutlist        = 'Setup default panel Layouts for themes in use. New and not yet formatted tracks will use the settings.\n\n'
                          ..'Note:\nThe default settings also affect other ReaperThemes that have A/B/C formatted layouts, and vice versa. '
helpR_dock              = 'Dock this script in full format. Best suited for the left or right side pane. '
helpR_trackLabelColor   = 'If a track has a custom color, use that color on the track name. '
helpR_resetCurrent      = 'This will Reset all parameters in the current theme back to its default settings. '
helpR_totalReset        = 'Removes ALL user changes for installed CSIX Series. Reloads current theme with default parameter values.\n '
                          ..'Custom Presets will not be reset. '
...
helpR_showThemesPath    = 'Navigates to the ColorThemes folder in current REAPER installation. '
...
helpR_trackNames        = 'AE and BC Series. '
helpR_buttonBrightness  = 'All theme series.\nSets the label brightness on ALL Action Buttons globally. '
helpR_custOverlay       = 'AE and BC Series.\nAdds an extra layer of custom color to all control panels (except on alternate full-tone layouts). '
...
helpR_indentPinTrack    = 'When ON, Pinned tracks will respect settings for Folder Indentation and Controls Alignment. '
...
helpR_trackInfo         = 'FOR DEV TESTING:\nAdds support for InfoBlock element if featured in REAPER. '
                          ..'\'SWAP\' will display InfoBlock only when Labels & Values are set to HIDE. \'PIN\' will always show Info (when size permits). '
helpR_showActBtnTrans   = 'Allow Action Buttons to be displayed in Transport Bar (when in single row, and when size permits).\n\n'
                          ..'Tip:  In CSIX AE and BC Series, the alternate layout \'TRANSPORT-2\' includes an additional button section (D). '
helpR_showMetroBtnTrans = 'Show Custom Buttons to extend the functionality of the Transport Bar.\n\n'
                          ..'Note: At this time, Custom Buttons cannot indicate current state. '
...
helpR_fxparmColumns     = 'For CSIX AE Series, layout B) and C):\nSets the number of COLUMNS if the FX LIST on the right side has an overflow.\n\n'
                          ..'Tip:\nThe second number (e.g. 1-2) equals two columns when track is minimized.\n\n'
                          ..'Note:\nThis setting is fixed for all other CSIX variants. '
...
helpR_masterCol         = 'AE/BC/MC Series\nColor Master Panel: Hold Shift to choose from swatch Palette, OR use the Picker.\n+Alt resets color. '
help_pref               = 'These buttons set REAPER preferences. Their settings are automatically saved to your REAPER install.'
help_proj_extmix=         'These settings are part of your REAPER project, and will be saved when it is saved '
                          ..'(except for \'Scroll to selected track\', which is a REAPER preference). '
helpR_recolProject      = 'Assigns random colors using the selected palette. Tracks which share a color will be given the same new color. '
helpR_colgradeChildren  = 'Sets stepped HSL GRADIENTS for all Child-tracks in folders AND sub-folders, based on the selected top Folder.\n\n'
                          ..'KEY MODIFIERS:\nClicking the icon while holding Shift will produce brighter tones. '
                          ..'Additionally, holding Ctrl (Cmd on Mac) will slow down the increments.  Hold Alt to apply without stepping. '
                          ..'Holding Ctrl+Alt+Shift applies the complementary color. '
helpR_choosePalette     = 'Click to select a palette. '
helpR_envMatchIndent    = 'Indent with folders alignment, matching the Indent and Align '
                          ..'setting on the Track Panel tab.'

--POPULATE --------------
apply = {}
root = Element:new(nil, {x=0,y=0,drawx=0,drawy=0,w=_gfxw,h=_gfxh,color={46,50,47}})  --fill
...
_replaceTheme = Button:new(_pageContainer, {x=0,y=-33,w=360,h=30,action=replaceTheme})
_theme = Element:new(_pageContainer, {x=0,y=0,w=360,h=30,text={str=''},helpR='Current Theme.'})

Button:new(_pageContainer, {x=-10,y=8,w=70,h=30,img='themesettings',imgType=3,action=toggleHelp,helpR=helpR_help})
_buttonHelp = Element:new(_pageContainer, {x=0,y=10,w=50,h=16,text={str='Help',style=1,align=5,col={170,170,170}}})
Button:new(_pageContainer, {x=50,y=8,w=70,h=30,img='themesettings',imgType=3,action=doDock,helpL=helpL_dock,helpR=helpR_dock})
Element:new(_pageContainer, {x=60,y=10,w=50,h=16,text={str='Dock',style=1,align=5,col={170,170,170}}})

_pageSpin = Spinner:new(_pageContainer, {spinStyle='image',valsImage='page_titles',x=135,y=0,w=243,h=40,action=doPageSpin})
Element:new(_pageContainer, {x=0,y=39,w=513,h=1,color={116,128,149,225}})  --title div
Button:new(_pageContainer, {x=360,y=-35,w=85,h=30,img='themesettings',imgType=3,action=csixListSwitch,helpR=helpR_switchCSIX})
Button:new(_pageContainer, {x=367,y=-33,w=70,h=30,text={str='SWITCH  >',style=3,align=2,col={180,180,181}}})
Button:new(_pageContainer, {x=440,y=-35,w=85,h=30,img='themesettings',imgType=3,action=csixListMerge,helpR=helpR_mergeCSIX})
Button:new(_pageContainer, {x=447,y=-33,w=70,h=30,text={str='MERGE  >',style=3,align=2,col={180,180,181}}})
Button:new(_pageContainer, {x=385,y=7,w=140,h=30,img='themesettings',imgType=3,action=csix_layout_list,helpR=helpR_layoutlist})
Button:new(_pageContainer, {x=377,y=9,w=140,h=30,text={str='Set default Layout  >',style=3,align=2,col={180,180,181}}})
...
_colAdjBox = Element:new(_colAdjBoxStroke, {x=1,y=1,w=511,h=527,color={38,42,39}})  --fill
...
Element:new(_colAdjBox, {x=69,y=90,w=80,h=11,text={str='GAMMA',style=1,align=4,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=69,y=178,w=80,h=11,text={str='HIGHLIGHTS',style=1,align=4,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=190,y=178,w=80,h=11,text={str='MIDTONES',style=1,align=6,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=320,y=178,w=80,h=11,text={str='SHADOWS',style=1,align=6,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=69,y=234,w=80,h=11,text={str='SATURATION',style=1,align=4,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=69,y=290,w=80,h=11,text={str='TINT',style=1,align=4,col={180,180,181}},helpR=helpR_colAdj})
...
Element:new(_colAdjBox, {x=96,y=316,w=190,h=30,text={str='Also affect project Custom Colors',style=1,align=4,col={180,180,181}}})
Button:new(_colAdjBox, {x=288,y=316,w=30,img='bin',imgType=3,action=resetColorControls,helpR=helpR_resetColAdj})
Element:new(_colAdjBox, {x=317,y=316,w=160,h=30,text={str='Reset / Read Color Controls',style=1,align=4,col={180,180,181}},helpR=helpR_resetColAdj})

Element:new(_pageGlobal_und, {x=0,y=10,w=513,h=357,helpL=helpL_layout})  --helpL

--theme-specific
Element:new(_colAdjBox, {x=30,y=356,w=450,h=1,color={116,128,149,225}})  --title div
Element:new(_colAdjBox, {x=0,y=368,w=511,h=20,text={str='Theme-Specific Additions:',style=3,align=5,col={136,136,136}}})

Element:new(_colAdjBox, {x=30,y=400,w=138,h=16,text={str='TRACK NAMES',style=1,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=30,y=418,w=138,h=16,text={str='Indicates Properties',style=1,align=5,col={160,160,161}}})
Spinner:new(_colAdjBox, {x=30,y=440,w=138,title='SHOW',action=paramSet,param='glb_trackLabelCol',valsTable=trackLabelColVals})

Element:new(_colAdjBox, {x=186,y=400,w=138,h=16,text={str='ACTION BUTTONS',style=1,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=186,y=418,w=138,h=16,text={str='* Label Brightness',style=1,align=5,col={160,160,161}}})
Spinner:new(_colAdjBox, {x=186,y=440,w=138,title='BRIGHT.',action=paramSet,param='ctrl_Param_201',valsTable=buttonBrightVals})

Element:new(_colAdjBox, {x=342,y=400,w=138,h=16,text={str='TCP/MCP TINT',style=1,align=5,col={180,180,181}}})
Element:new(_colAdjBox, {x=342,y=418,w=138,h=16,text={str='* Custom Color',style=1,align=5,col={160,160,161}}})
...
Element:new(_colAdjBox, {x=30,y=400,w=138,h=70,helpL=helpL_layout,helpR=helpR_trackNames})
Element:new(_colAdjBox, {x=186,y=400,w=138,h=70,helpL=helpL_layout,helpR=helpR_buttonBrightness})
Element:new(_colAdjBox, {x=342,y=400,w=138,h=70,helpL=helpL_layout,helpR=helpR_custOverlay})

Element:new(_colAdjBox, {x=30,y=490,w=450,h=1,color={116,128,149,225}})  --title div

Button:new(_colAdjBox, {x=0,y=496,w=170,h=30,img='themesettings',imgType=3,action=resetCurrent,helpR=helpR_resetCurrent})
Element:new(_colAdjBox, {x=0,y=498,w=170,h=16,text={str='Reset This Theme',style=1,align=5,col={210,106,106}}})
Button:new(_colAdjBox, {x=340,y=496,w=170,h=30,img='themesettings',imgType=3,action=totalReset,helpR=helpR_totalReset})
Element:new(_colAdjBox, {x=340,y=498,w=170,h=16,text={str='Total Reset CSIX',style=1,align=5,col={210,106,106}}})

--csix settings
Button:new(_pageGlobal_und, {x=0,y=546,w=170,h=30,img='themesettings',imgType=3,action=exportParams,helpR=helpR_csixSetChange})
Element:new(_pageGlobal_und, {x=0,y=548,w=170,h=16,text={str='SAVE CHANGES',style=1,align=5,col={180,180,181}}})
Button:new(_pageGlobal_und, {x=170,y=546,w=170,h=30,img='themesettings',imgType=3,action=importParams,helpR=helpR_csixSetImport})
Element:new(_pageGlobal_und, {x=170,y=548,w=170,h=16,text={str='IMPORT SETTINGS *',style=1,align=5,col={180,180,181}}})
Button:new(_pageGlobal_und, {x=170,y=572,w=170,h=30,img='themesettings',imgType=3,action=showSettingPath,helpR=helpR_showPresets})
Element:new(_pageGlobal_und, {x=170,y=574,w=170,h=16,text={str='Show Presets',style=1,align=5,col={180,180,181}}})
Button:new(_pageGlobal_und, {x=340,y=546,w=170,h=30,img='themesettings',imgType=3,action=exportParamsAll,helpR=helpR_csixSetAll})
Element:new(_pageGlobal_und, {x=340,y=548,w=170,h=16,text={str='SAVE ALL SETTINGS',style=1,align=5,col={180,180,181}}})
Button:new(_pageGlobal_und, {x=0,y=572,w=170,h=30,img='themesettings',imgType=3,action=importCsixSet,helpR=helpR_csixsetLoad})
Element:new(_pageGlobal_und, {x=0,y=574,w=170,h=16,text={str='Load Changes',style=1,align=5,col={180,180,181}}})
Button:new(_pageGlobal_und, {x=340,y=572,w=170,h=30,img='themesettings',imgType=3,action=importCsixSetAll,helpR=helpR_csixsetLoadAll})
Element:new(_pageGlobal_und, {x=340,y=574,w=170,h=16,text={str='Load All',style=1,align=5,col={180,180,181}}})
...
_tcpBtnOlHelp = Element:new(_pageTrack_und, {x=92,y=60,w=154,h=86,visible=false,helpR=helpR_fallbackA})  --basic narrow only
...
Element:new(_pageTrack_und, {x=56,y=30,w=125,h=30,text={str='Indent Pinned *',style=1,align=4,col={180,180,181}},helpR=helpR_indentPinTrack,helpL=helpL_layout})
...
Element:new(_pageTrack_und, {x=284,y=40,w=25,h=30,text={str='FX',style=1,align=4,col={180,180,181}},helpR=helpR_showTcpFx,helpL=help_pref})
...
Element:new(_pageTrack_und, {x=326,y=40,w=125,h=30,text={str='FX PARM (ALWAYS) *',style=1,align=4,col={180,180,181}},helpR=helpR_showTcpParm,helpL=helpL_layout})
...
Element:new(_pageTrack_und, {x=465,y=40,w=50,h=30,text={str='SENDS',style=1,align=4,col={180,180,181}},helpR=helpR_showTcpSends,helpL=help_pref})

_layoutTrackStroke = Element:new(_pageTrack_und, {x=0,y=153,w=513,h=496,color={116,128,149,225}})  --stroke

_tcpButLayA = Button:new(_pageTrack_und, {x=20,y=65,w=69,h=89,img='layout_select_A',imgType=3,action=doActiveLayout,param={'tcp','A'},helpR=helpR_layoutButton})
_tcpButLayB = Button:new(_pageTrack_und, {x=93,y=65,w=69,h=89,img='layout_select_B',imgType=3,action=doActiveLayout,param={'tcp','B'},helpR=helpR_layoutButton})
_tcpButLayC = Button:new(_pageTrack_und, {x=166,y=65,w=69,h=89,img='layout_select_C',imgType=3,action=doActiveLayout,param={'tcp','C'},helpR=helpR_layoutButton})
...
_tcpTrackSound_A = Button:new(_pageTrack_und, {x=37,y=86,w=39,h=15, img='tracksound', imgType=3, action=doTrackSound, param={'tcp','A'}, signalState=0})  --tcp trackSound
_tcpTrackSound_B = Button:new(_pageTrack_und, {x=110,y=86,w=39,h=15, img='tracksound', imgType=3, action=doTrackSound, param={'tcp','B'}, signalState=0})
_tcpTrackSound_C = Button:new(_pageTrack_und, {x=183,y=86,w=39,h=15, img='tracksound', imgType=3, action=doTrackSound, param={'tcp','C'}, signalState=0})

_tcpBtnOverlay = Element:new(_pageTrack_und, {x=92,y=60,w=154,h=86,color={46,50,47,170},visible=false,helpR=helpR_fallbackA})  --basic narrow only

_applyStroke_Track = Element:new(_pageTrack_und, {x=254,y=70,w=259,h=83,color={116,128,149,225}})  --stroke
_applyBox_Track = Element:new(_applyStroke_Track, {x=1,y=1,w=257,h=83,color={46,50,47},helpL=helpL_applySize})  --fill
Element:new(_applyStroke_Track, {x=17,y=16,w=213,h=67,color={0,0,0,35},helpL=helpL_applySize})  --fill
apply.und = {tcp={Button:new(_applyBox_Track, {x=60,y=25,w=61,h=23,img='apply_100',imgType=3,action=applyLayout,param={'tcp',''},helpR=helpR_applySize,helpL=helpL_applySize})}}
apply.und.tcp[2] = Button:new(_applyBox_Track, {x=90,y=25,w=61,h=23,img='apply_150',imgType=3,action=applyLayout,param={'tcp','150%_'},helpR=helpR_applySize,helpL=helpL_applySize,visible=false})  --NA
apply.und.tcp[3] = Button:new(_applyBox_Track, {x=123,y=25,w=61,h=23,img='apply_200',imgType=3,action=applyLayout,param={'tcp','200%_'},helpR=helpR_applySize,helpL=helpL_applySize})
Element:new(_applyBox_Track, {x=21,y=60,w=200,h=10,img='apply_to_sel'})

_layoutTrack = Element:new(_layoutTrackStroke, {x=1,y=1,w=511,h=494,color={46,50,47},helpL=helpL_layout})  --fill
_tcpPageOverlay = Element:new(_layoutTrack, {x=0,y=0,w=511,h=494,color={200,100,100,20},visible=false,helpR=helpR_fallbackA})  --basic narrow only
...
Spinner:new(_layoutTrack, {x=190,y=82,w=129,title='TRACK INFO *',action=paramSet,param='ctrl_Param_101',valsTable=tcpTrackInfoVals,helpR=helpR_trackInfo,helpL=helpL_layout})
...
_trackTable = ParamTable:new(_layoutTrack, {x=29,y=121,w=453,h=367,valsTable=tcpTableVals})
Element:new(_trackTable, {x=0,y=0,w=115,h=30,text={str='HIDE:',style=1,align=6,col={140,140,140}}})
...
_mcpButLayA = Button:new(_pageMixer_und, {x=30,y=45,w=69,h=89,img='layout_select_A_blue',imgType=3,action=doActiveLayout,param={'mcp','A'},helpR=helpR_layoutButton})
_mcpButLayB = Button:new(_pageMixer_und, {x=103,y=45,w=69,h=89,img='layout_select_B_blue',imgType=3,action=doActiveLayout,param={'mcp','B'},helpR=helpR_layoutButton})
_mcpButLayC = Button:new(_pageMixer_und, {x=176,y=45,w=69,h=89,img='layout_select_C_blue',imgType=3,action=doActiveLayout,param={'mcp','C'},helpR=helpR_layoutButton})
...
_mcpTrackSound_A = Button:new(_pageMixer_und, {x=47,y=66,w=39,h=15, img='tracksound', imgType=3, action=doTrackSound, param={'mcp','A'}, signalState=0})  --mcp TrackSound
_mcpTrackSound_B = Button:new(_pageMixer_und, {x=120,y=66,w=39,h=15, img='tracksound', imgType=3, action=doTrackSound, param={'mcp','B'}, signalState=0})
_mcpTrackSound_C = Button:new(_pageMixer_und, {x=193,y=66,w=39,h=15, img='tracksound', imgType=3, action=doTrackSound, param={'mcp','C'}, signalState=0})
...
_applyBox_Mixer = Element:new(_applyStroke_Mixer, {x=1,y=1,w=237,h=83,color={46,50,47},helpL=helpL_applySize})  --fill
Element:new(_applyStroke_Mixer, {x=17,y=16,w=205,h=67,color={0,0,0,35},helpL=helpL_applySize})  --fill
apply.und.mcp = {Button:new(_applyBox_Mixer, {x=54,y=25,w=61,h=23,img='apply_100',imgType=3,action=applyLayout,param={'mcp',''},helpR=helpR_applySize,helpL=helpL_applySize})}
apply.und.mcp[2] = Button:new(_applyBox_Mixer, {x=90,y=25,w=61,h=23,img='apply_150',imgType=3,action=applyLayout,param={'mcp','150%_'},helpR=helpR_applySize,helpL=helpL_applySize,visible=false})  --NA
apply.und.mcp[3] = Button:new(_applyBox_Mixer, {x=115,y=25,w=61,h=23,img='apply_200',imgType=3,action=applyLayout,param={'mcp','200%_'},helpR=helpR_applySize,helpL=helpL_applySize})
Element:new(_applyBox_Mixer, {x=17,y=60,w=200,h=10,img='apply_to_sel'})

_mixerTop = Element:new(_mixerTopStroke, {x=1,y=1,w=511,h=328,color={46,50,47},helpL=helpL_layout})  --fill
...
_mixerTable = ParamTable:new(_mixerTop, {x=29,y=122,w=453,h=199,valsTable=mcpTableVals})
Element:new(_mixerTable, {x=0,y=0,w=115,h=30,text={str='SHOW:',style=1,align=6,col={140,140,140}}})

_mMid = Element:new(_pageMixer_und, {x=0,y=468,w=513,h=31,color={116,128,149,225}})  --stroke
_genMixer = Element:new(_mMid, {x=1,y=1,w=511,h=29,color={38,42,39},helpL=help_proj_extmix})  --fill
Button:new(_genMixer, {x=23,y=0,w=30,img='check-buttons_check',imgType=3,action=actionToggle,param=40371})
Element:new(_genMixer, {x=50,y=1,w=160,h=30,text={str='Show Multiple-Row in Mixer',style=1,align=4,col={180,180,181}}})
Button:new(_genMixer, {x=213,y=0,w=30,img='check-buttons_check',imgType=3,action=actionToggle,param=40221})
Element:new(_genMixer, {x=240,y=1,w=140,h=30,text={str='Scroll to Selected Track',style=1,align=4,col={180,180,181}}})
Button:new(_genMixer, {x=389,y=0,w=30,img='check-buttons_check',imgType=3,action=actionToggle,param=40903})
Element:new(_genMixer, {x=416,y=1,w=80,h=30,text={str='Show Icons',style=1,align=4,col={180,180,181}}})

_mLower = Element:new(_pageMixer_und, {x=0,y=504,w=513,h=145,color={116,128,149,225}})  --stroke
_extMixerf = Element:new(_mLower, {x=1,y=1,w=255,h=143,color={38,42,39},helpL=help_proj_extmix})  --fill
Element:new(_extMixerf, {x=29,y=8,w=257,h=20,text={str='EXTENDED MIXER :',style=1,align=0,col={180,180,181}}})
Button:new(_extMixerf, {x=23,y=21,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=40549})
Element:new(_extMixerf, {x=50,y=21,w=190,h=30,text={str='Show FX Inserts',style=1,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=23,y=50,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=40910})
Element:new(_extMixerf, {x=50,y=50,w=190,h=30,text={str='Show FX Parameters',style=1,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=48,y=67,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=41829})
Element:new(_extMixerf, {x=74,y=67,w=190,h=30,text={str='Group with their Inserts',style=1,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=23,y=94,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=40557})
Element:new(_extMixerf, {x=50,y=94,w=190,h=30,text={str='Show Sends',style=1,align=4,col={180,180,181}}})
Button:new(_extMixerf, {x=48,y=111,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=40267})
Element:new(_extMixerf, {x=74,y=111,w=190,h=30,text={str='Group below/after Inserts',style=1,align=4,col={180,180,181}}})

_mastMixer = Element:new(_mLower, {x=256,y=1,w=256,h=143,color={46,50,47}})  --fill
Element:new(_mastMixer, {x=22,y=8,w=257,h=20,text={str='MASTER MIXER :',style=1,align=0,col={180,180,181}}})

Button:new(_mastMixer, {x=16,y=21,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=40389})
Element:new(_mastMixer, {x=45,y=21,w=190,h=30,text={str='Show Master on Right Side',style=1,align=4,col={180,180,181}}})
Button:new(_mastMixer, {x=16,y=45,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=41610})
Element:new(_mastMixer, {x=45,y=45,w=190,h=30,text={str='Toggle in Dock',style=1,align=4,col={180,180,181}}})
Button:new(_mastMixer, {x=16,y=67,w=30,img='check-buttons_check_blue',imgType=3,action=actionToggle,param=41636})
Element:new(_mastMixer, {x=45,y=67,w=190,h=30,text={str='Toggle in Floating Window',style=1,align=4,col={180,180,181}}})
...
Element:new(_mastMixer, {x=45,y=89,w=190,h=30,text={str='Always Show Meter Scales *',style=1,align=4,col={180,180,181}},helpR=helpR_mastermeter})
...
Element:new(_mastMixer, {x=45,y=111,w=190,h=30,text={str='Wide Meter (for AE Master W) *',style=1,align=4,col={180,180,181}},helpR=helpR_masterw})
...
_paletteBox = Element:new(_paletteBoxStroke, {x=1,y=1,w=511,h=225,color={46,50,47},helpL=helpL_customCol})  --fill

_masterBox = Element:new(_paletteBox, {x=400,y=142,w=111,h=60,helpR=helpR_masterCol})
Element:new(_masterBox, {x=30,y=9,w=6,h=6,draw=drawMasterColorSwatch})  --current color
Button:new(_masterBox, {x=26,y=6,w=30,h=30,img='color_picker_main',imgType=3,action=MasterCustomColor})
Element:new(_masterBox, {x=0,y=40,w=84,h=20,text={str='Color Master *',style=1,align=9,col={180,180,181}}})
...
Element:new(_paletteBox, {x=30,y=110,w=450,h=20,text={str='Hold & click:    Ctrl/Cmd for ITEMS.    Ctrl/Cmd + Alt for TAKES.    Shift for MASTER.',style=1,align=5,col={180,180,181}}})
--Set project colors
Button:new(_paletteBox, {x=27,y=140,w=30,img='color_apply_all',imgType=3,action=applyPalette,helpR=helpR_recolProject})
Element:new(_paletteBox, {x=62,y=140,w=170,h=30,text={str='PROJECT: Recolor All Tracks *',style=1,align=4,col={180,180,181}},helpR=helpR_recolProject})
...
Element:new(_paletteBox, {x=62,y=180,w=170,h=30,text={str='HSL Gradient to CHILDREN *',style=1,align=4,col={180,180,181}},helpR=helpR_colgradeChildren})
...
Element:new(_paletteBox, {x=272,y=140,w=130,h=30,text={str='ITEMS: Set to Default',style=1,align=4,col={180,180,181}}})
...
Element:new(_paletteBox, {x=272,y=180,w=130,h=30,text={str='TAKES: Set to Default',style=1,align=4,col={180,180,181}}})

_paletteMenuBox = Element:new(_paletteBoxStroke, {x=1,y=222,w=511,h=272,color={38,42,39}})  --fill
...
Element:new(_pageColor_und, {x=63,y=507,w=210,h=30,text={str='Dim all Assigned Custom Colors',style=1,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=28,y=545,w=30,img='color_dim_all',imgType=3,action=reduceCustCol,param=true})
Element:new(_pageColor_und, {x=63,y=545,w=210,h=30,text={str='Dim Colors on Selected Tracks',style=1,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=28,y=584,w=30,img='bin',imgType=3,action=setTrackDefaultColor})
Element:new(_pageColor_und, {x=63,y=583,w=210,h=30,text={str='RESET Selected Tracks to Default Col.',style=1,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=280,y=507,w=30,img='color_picker',imgType=3,action=TrackCustomColor})
Element:new(_pageColor_und, {x=313,y=507,w=210,h=30,text={str='Track: Set to Custom Color (Picker)',style=1,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=280,y=545,w=30,img='color_random',imgType=3,action=TrackRandomColor})
Element:new(_pageColor_und, {x=313,y=545,w=210,h=30,text={str='Track: Set to Random Colors (16)',style=1,align=4,col={180,180,181}}})

Button:new(_pageColor_und, {x=280,y=584,w=30,img='bin',imgType=3,action=resetRandomGenerator})
Element:new(_pageColor_und, {x=313,y=583,w=210,h=30,text={str='Color: Reset Random Generator',style=1,align=4,col={180,180,181}}})


--ENV & TRANSPORT PAGE
_pageEnvTrans_und = Element:new(_subPageContainer, {x=0,y=40,w=513,h=639,helpL=helpL_layout})
apply.und.envcp = {Button:new(_pageEnvTrans_und, {x=186,y=2,w=61,h=23,img='apply_100',imgType=3,action=applyLayout,param={'envcp',''},helpL=helpL_applySize})}
apply.und.envcp[2] = Button:new(_pageEnvTrans_und, {x=228,y=2,w=61,h=23,img='apply_150',imgType=3,action=applyLayout,param={'envcp','150%_'},helpL=helpL_applySize,visible=false})  --NA
apply.und.envcp[3] = Button:new(_pageEnvTrans_und, {x=270,y=2,w=61,h=23,img='apply_200',imgType=3,action=applyLayout,param={'envcp','200%_'},helpL=helpL_applySize})
...
Button:new(_pageEnvTrans_und, {flow=false,x=150,y=104,w=30,img='check-buttons_check_purple',imgType=3,action=paramToggle,param='envcp_folder_indent',helpR=helpR_envMatchIndent})
Element:new(_pageEnvTrans_und, {x=178,y=104,w=200,h=30,text={str='Match Track Folder Indent/Align',style=3,align=4,col={180,180,181}},helpR=helpR_envMatchIndent})
...
apply.und.trans = {Button:new(_pageEnvTrans_und, {x=186,y=278,w=61,h=23,img='apply_100',imgType=3,action=applyLayout,param={'trans',''},helpL=helpL_applySize})}
apply.und.trans[2] = Button:new(_pageEnvTrans_und, {x=228,y=278,w=61,h=23,img='apply_150',imgType=3,action=applyLayout,param={'trans','150%_'},helpL=helpL_applySize,visible=false})  --NA
apply.und.trans[3] = Button:new(_pageEnvTrans_und, {x=270,y=278,w=61,h=23,img='apply_200',imgType=3,action=applyLayout,param={'trans','200%_'},helpL=helpL_applySize})

Element:new(_pageEnvTrans_und, {x=155,y=322,w=200,h=16,text={str='* See Help text >',style=3,align=5,col={160,160,161}}})
...
_transPrefs = Element:new(_transPrefsStroke, {x=1,y=1,w=511,h=174,color={38,42,39},helpL=help_pref})  --fill

Button:new(_transPrefs, {x=29,y=26,img='check-buttons_check_purple',imgType=3,action=actionToggle,param=40533})
Element:new(_transPrefs, {x=68,y=26,w=150,h=30,text={str='Center Transport',style=1,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=29,y=69,img='check-buttons_check_purple',imgType=3,action=actionToggle,param=40531})
Element:new(_transPrefs, {x=68,y=69,w=150,h=30,text={str='Show Play Rate',style=1,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=29,y=112,img='check-buttons_check_purple',imgType=3,action=actionToggle,param=40680})
Element:new(_transPrefs, {x=68,y=112,w=170,h=30,text={str='Show Time Signature',style=1,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=26,img='check-buttons_check_purple',imgType=3,action=actionToggle,param=40868})
Element:new(_transPrefs, {x=324,y=26,w=170,h=30,text={str='Use Home/End for Markers',style=1,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=69,img='check-buttons_check_purple',imgType=3,action=actionToggle,param=40532})
Element:new(_transPrefs, {x=324,y=69,w=170,h=30,text={str='Show Play State as Text',style=1,align=4,col={180,180,181}}})
Button:new(_transPrefs, {x=285,y=112,img='check-buttons_check_purple',imgType=3,action=actionToggle,param=40620})
Element:new(_transPrefs, {x=324,y=112,w=170,h=30,text={str='External Timecode Sync.',style=1,align=4,col={180,180,181}}})


--MyMod PAGE
_pageMyMod_und = Element:new(_subPageContainer, {x=0,y=0,w=513,h=639})
_myModBoxFrame = Element:new(_pageMyMod_und, {x=0,y=10,w=513,h=529,color={116,128,149,225}})
_myModBox = Element:new(_myModBoxFrame, {x=1,y=1,w=511,h=527,color={38,42,39}})
Element:new(_myModBox, {x=10,y=100,w=491,h=300, text={str=
'Due to file size limits, this basic version of CSIX Theme Adjuster \n\ndoes not include the MyMod feature.\n\n\n\n\n'
..'MyMod provides extended control over themes in the CSIX Series, \n\nenabling easy creation of editable versions for customizing \n\n'
..'action buttons, configuration and visuals.\n\n\n\n\n' 
..'Try out MyMod in the full Adjuster version,\n\navailable for free on the CSIX website.' ,
style=3,align=3,col={180,180,181}}})
Button:new(_pageMyMod_und, {x=150,y=609,w=205,h=40,img='themelogo',imgType=3,action=open_url_csix})

----------------------------------------
--HELP
_helpL = Element:new(_pageContainer, {x=-152,y=500,w=115,h=160,text={str='',style=1,align=2,wrap=true,vCenter=false,lineSpacing=14,col={170,170,204}}})
Element:new(_helpL, {x=18,y=-25,w=97,h=19,img='helpHeader_l'})
_helpR = Element:new(_pageContainer, {x=550,y=200,w=115,h=160,text={str='',style=1,align=0,wrap=true,vCenter=false,lineSpacing=14,col={170,170,204}}})
Element:new(_helpR, {x=0,y=-25,w=97,h=19,img='helpHeader_r'})

_systemInfo = Element:new(_pageContainer, {x=0,y=-0,w=150,h=80, helpR='System info.', visible = false,})
Button:new(_systemInfo, {x=0,y=0,w=150,h=60,text={str=''..reaper.GetAppVersion(),style=1,align=2,col={136,136,136}}})
Button:new(_systemInfo, {x=0,y=14,w=120,h=15,text={str=''.. OS .. '  –',style=1,align=6,col={136,136,136}}})
Button:new(_systemInfo, {x=110,y=14,w=40,h=15,img='dpiScale',imgType=3})
_csixVersion = Button:new(_systemInfo, {x=0,y=30,w=150,h=16,text={str='',style=1,align=2,col={136,136,136}},func=function(self) end})  --csix_version
Element:new(_systemInfo, {x=0,y=46,w=150,h=16,text={str='CSIX_adjuster 3.6.0',style=1,align=2,col={136,136,136}}})
Element:new(_systemInfo, {x=0,y=62,w=150,h=16,text={str='(Basic edition)',style=1,align=2,col={100,160,120}}})
...
chgsel = 1
oldTheme = nil
isGenericTheme = false  --custom adjuster
mouseXold = 0
mouseYold = 0
...
function runloop()
  ...
  if currentTrack ~= lastSelectedTrack then
    ...
    updateTrackSound()  --on selection
    updateTcpPageOverlay()  --basic narrow
    updateTcpBtnOlHelp()  --basic narrow
    updateTcpBtnOverlay()  --basic narrow
  end
